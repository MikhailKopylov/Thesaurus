...
fun <T : View> Activity.viewById(@IdRes viewId: Int): ViewById<T> {
    return ViewById({ window.decorView.findViewById(android.R.id.content) }, viewId)
}

fun <T : View> Fragment.viewById(@IdRes viewId: Int): ViewById<T> {
    return ViewById({ view }, viewId)
}
